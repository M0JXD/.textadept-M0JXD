...
M.collapsed_folder_color = 'keyword'
M.expanded_folder_color = 'number'
M.files_color = 'default'
...
local function highlight_folder(start_line)
	...
	local expanded_style = buffer:style_of_name(M.expanded_folder_color)
	local collapsed_style = buffer:style_of_name(M.collapsed_folder_color)
	...
	for i = start_line and start_line or 1, buffer.line_count do
		...
		if line:find('[/\\][\r\n]*$') then
			...
			-- Files are unstyled
			buffer:start_styling(buffer:position_from_line(i), 0)
			buffer:set_styling(#line, buffer:style_of_name(M.files_color))
		end
	end
end
